...
const S3_SYNC_CACHE_COLUMN_MIGRATIONS = [
  ['content_type', `ALTER TABLE s3_sync_cache ADD COLUMN content_type TEXT`],
];
...
function ensureS3SyncCacheColumns(db) {
  const columns = db.prepare('PRAGMA table_info(s3_sync_cache)').all().map((row) => row.name);
  for (const [name, sql] of S3_SYNC_CACHE_COLUMN_MIGRATIONS) {
    if (!columns.includes(name)) {
      db.exec(sql);
    }
  }
...
export function getDb() {
  ...
  _db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS s3_sync_cache (
    ...
    CREATE INDEX IF NOT EXISTS idx_s3_sync_cache_s3_key ON s3_sync_cache(s3_key);
  `);

  ensureTaskColumns(_db);
  ensureS3SyncCacheColumns(_db);
