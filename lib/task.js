...
function repairWorkspacePathIfNeeded(task) {
  if (!task?.id) return task;

  const resolvedWorkspacePath = resolveWorkspacePath(task);
  if (!resolvedWorkspacePath || resolvedWorkspacePath === task.workspace_path) {
    return task;
  }

  const db = getDb();
  db.prepare('UPDATE tasks SET workspace_path = ? WHERE id = ?')
    .run(resolvedWorkspacePath, task.id);

  const updatedTask = {
    ...task,
    workspace_path: resolvedWorkspacePath,
  };
  syncTaskJson(resolvedWorkspacePath, updatedTask);
  return updatedTask;
}
...
export function getTask(id) {
  ...
  return normalizeTaskRecord(repairWorkspacePathIfNeeded(task));
}

function persistWorkspace(task) {
  const workspacePath = task.workspace_path && existsSync(task.workspace_path)
    ? task.workspace_path
    : resolveWorkspacePath(task) || createWorkspace(task.id, task);
  const db = getDb();
...
export function setStatus(id, newStatus, message) {
  ...
  const updated = persistWorkspace(getTask(id));

  // Async sync: local resource cache + optional S3
  if (newStatus === 'done' && updated.workspace_path) {
    Promise.all([
      import('./resource-cache.js'),
      import('./webui/config-store.js'),
    ]).then(([{ cacheExternalResources }, { readWebuiConfig }]) => {
      const config = readWebuiConfig();
      if (config.resourceCache?.enabled === false) {
        return null;
      }
      return cacheExternalResources(updated.workspace_path);
    }).catch(err => {
      console.error(`[sync] Failed for task ${id}:`, err.message);
    });
  }
...
export function listTasks({ status, all } = {}) {
  ...
  if (status && status !== 'all') {
    return db.prepare('SELECT * FROM tasks WHERE status = ? ORDER BY updated_at DESC')
      .all(status)
      .map(repairWorkspacePathIfNeeded)
