...
export function listTasks({ status, all } = {}) {
  ...
  if (status && status !== 'all') {
    return db.prepare('SELECT * FROM tasks WHERE status = ? ORDER BY updated_at DESC').all(status);
  }
  if (!all) {
    return db.prepare('SELECT * FROM tasks WHERE status != ? ORDER BY updated_at DESC').all('archived');
  }
